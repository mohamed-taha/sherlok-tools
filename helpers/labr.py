...
class LABR:
    def __init__(self):
        self.REVIEWS_PATH = "../datasets/labr_data/"
...
#            if review[0] == u"#": continue
    ...
    def split_train_test_2class(self, rating, percent_test,
        ...
        if balanced == "unbalanced":
            ...
            test_ids = review_ids[:ntest]  #بياخد اول كذا كتيست
            train_ids = review_ids[ntest:]
...
features_train,labels_train,features_test,labels_test=l.get_train_test()
...
print clf.predict(features_test_transformed[100])
print clf.score(features_test_transformed, labels_test)

from sklearn.svm import LinearSVC
clf2=LinearSVC()
clf2.fit(features_train_transformed,labels_train)
print(clf2.predict(features_test_transformed[100]))
print(clf2.score(features_test_transformed,labels_test))
...
(rating, a, b, c, body)=l.read_clean_reviews()
print a[1] ,b[1] ,c[1]
train,test=l.split_train_validation_test_3class_tiny(rating,0.5, 0.2, 0.2)
...
for i in range(0,len(train)):
    features_train.append(body[i].strip())
    labels_train.append(rating[i])
for j in range(0,len(test)):
    features_test.append(body[j].strip())
    labels_test.append(rating[j])
...
clf=GaussianNB()
clf.fit(features_train_transformed,labels_train)
print clf.predict(features_test_transformed[50])
#print clf.score(features_test_transformed, labels_test)
'''